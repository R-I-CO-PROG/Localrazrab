...
exports.parseItemCountBounds = parseItemCountBounds;
exports.parseDesiredItemCount = parseDesiredItemCount;
exports.defaultItemCount = defaultItemCount;
function normalizeCountText(userPrompt) {
...
function parseSingleItemCount(text) {
    ...
    const wordQty = text.match(/(?:^|[\s,.:;])(один|одна|одну|два|две|три|четыре|пять|шесть|семь|восемь)[\s]+(?:товар|позици|предмет|вид|sku|круж|бутыл|издел)/i);
    if (wordQty) {
...
function parseItemCountBounds(userPrompt) {
    ...
    const minOnly = text.match(/(?:минимум|как\s+минимум|не\s+менее|от)\s+(\d+)\s*(?:товар\w*|позици\w*|предмет\w*|издел\w*|составляющ\w*|компонент\w*|штук\w*|вид\w*)?/i);
    if (minOnly) {
        const min = parseInt(minOnly[1], 10);
        if (min >= 1 && min <= 10) {
            const maxMatch = text.match(/(?:до|максимум|не\s+более)\s+(\d+)\s*(?:товар|позици|предмет|издел)/i);
            const max = maxMatch ? parseInt(maxMatch[1], 10) : Math.min(10, min + 2);
...
function parseDesiredItemCount(userPrompt) {
...
function defaultItemCount(userPrompt) {
    return parseDesiredItemCount(userPrompt) ?? 4;
}
//# sourceMappingURL=parse-desired-count.js.map