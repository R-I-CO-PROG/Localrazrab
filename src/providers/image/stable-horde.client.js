...
exports.stableHordeGenerate = stableHordeGenerate;
const common_1 = require("@nestjs/common");
async function stableHordeGenerate(opts) {
    const logger = opts.logger ?? new common_1.Logger('StableHordeClient');
    ...
    const submitRes = await fetchWithRetry(`${baseUrl}/generate/async`, { method: 'POST', headers, body: JSON.stringify(body) }, logger);
    if (!submitRes.ok) {
    ...
    while (Date.now() - started < timeoutMs) {
        ...
        if (check.faulted)
            throw new Error('Stable Horde: job faulted');
    ...
    if (!done)
        throw new Error('Stable Horde: timeout waiting for generation');
    ...
    if (!img)
        throw new Error('Stable Horde: empty image');
...
async function fetchWithRetry(url, init, logger, retries = 4) {
    let lastError;
...
//# sourceMappingURL=stable-horde.client.js.map