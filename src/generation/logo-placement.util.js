...
exports.logoAnchorForProductName = logoAnchorForProductName;
exports.getLogoPlacementsForLifestyleScene = getLogoPlacementsForLifestyleScene;
exports.getLogoPlacementsForProducts = getLogoPlacementsForProducts;
const mockup_layout_1 = require("./mockup-layout");
const logo_surface_util_1 = require("./logo-surface.util");
function logoAnchorForProductName(name) {
    const n = name.toLowerCase();
    if (n.includes('кепк') || n.includes('бини') || n.includes('шапк')) {
        return { cx: 0.5, cy: 0.42, maxScale: 0.16 };
    }
    if (n.includes('блокнот') ||
        n.includes('блок для') ||
        n.includes('graphite') ||
        n.includes('графит') ||
    ...
    if (n.includes('носок'))
        return { cx: 0.5, cy: 0.34, maxScale: 0.12 };
    if (n.includes('шарф'))
    ...
    if (n.includes('ручк') || n.includes('карандаш'))
        return { cx: 0.5, cy: 0.45, maxScale: 0.095 };
...
function getLogoPlacementsForLifestyleScene(productNames) {
    if (productNames.length === 0)
        return [];
    const total = productNames.length;
    const placements = productNames.map((name, i) => {
        ...
        return {
            ...
            productName: name,
            surface: (0, logo_surface_util_1.detectLogoSurface)(name),
        };
    });
...
function getLogoPlacementsForProducts(productNames, width = 1024, height = 1024) {
    if (productNames.length === 0)
        return [];
    const cells = (0, mockup_layout_1.computeSceneTablePositions)(productNames.length, width, height);
...
//# sourceMappingURL=logo-placement.util.js.map