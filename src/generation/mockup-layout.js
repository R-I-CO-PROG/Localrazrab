...
exports.computeSceneTablePositions = computeSceneTablePositions;
exports.computeProductGrid = computeProductGrid;
exports.getLogoPlacementsForScene = getLogoPlacementsForScene;
exports.logoPlacementsFromGrid = logoPlacementsFromGrid;
exports.getStudioLogoPlacements = getStudioLogoPlacements;
function computeSceneTablePositions(count, width, height) {
    if (count === 1) {
...
function computeProductGrid(count, width, height) {
    const padding = 56;
    const footerH = 36;
...
function isClothingName(name) {
    const n = name.toLowerCase();
    return (n.includes('футбол') ||
        n.includes('поло') ||
        n.includes('худи') ||
        n.includes('свитшот') ||
        n.includes('кепк') ||
...
function getLogoPlacementsForScene(count, productNames = [], width = 1024, height = 1024) {
    if (count <= 0)
        return [];
    const allClothing = productNames.length === count && productNames.every((n) => isClothingName(n));
    if (allClothing) {
...
function logoPlacementsFromGrid(count, width, height) {
...
function getStudioLogoPlacements(count) {
    const presets = {
...
//# sourceMappingURL=mockup-layout.js.map