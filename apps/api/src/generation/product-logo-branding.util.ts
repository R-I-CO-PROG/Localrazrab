...
/** Способ и зона нанесения логотипа по типу товара (для image/LLM промптов) */
export function getProductLogoBranding(productNameRu: string): ProductLogoBranding {
  const n = productNameRu.toLowerCase();
  const surface = detectLogoSurface(productNameRu);
...
export function buildPerProductLogoBrandingLine(productNameRu: string, index: number): string {
  const b = getProductLogoBranding(productNameRu);
  return `Item ${index + 1} (${productNameRu}): ${b.methodEn} on ${b.placementEn}`;
}

export function buildPerProductLogoBrandingBlock(productNames: string[]): string {
  if (productNames.length === 0) return '';
  return productNames.map((name, i) => buildPerProductLogoBrandingLine(name, i)).join('; ');
}

/** Креативный AI — тот же промпт логотипа, что в каталоге (без фото SKU, только реф логотипа) */
export function buildCreativeAiLogoReferencePrompt(
  productNames: string[],
  options?: { logoHint?: string },
): string {
...
/** Креативная сцена без списка товаров — логотип с референса на главный объект */
export function buildCreativeLogoApplicationPrompt(logoHint?: string): string {
...
/** Общие negative-фразы для логотипа (каталог + креатив) */
export const CATALOG_LOGO_NEGATIVE_PROMPT =
...
/** Текст непосредственно перед картинкой логотипа в OpenRouter (снижает «вставку» рефа в угол кадра) */
export const OPENROUTER_LOGO_REF_PREAMBLE =
...
/** Дополнение для каталожной welcome-box сцены */
export const OPENROUTER_CATALOG_LOGO_REF_PREAMBLE =
  `${OPENROUTER_LOGO_REF_PREAMBLE} Apply logo ONLY on visible merchandise inside the box (apparel, cap, bottle, etc.) — NEVER on the presentation box cardboard, foam insert, or packaging exterior.`;

/** Общие avoid-фразы при hasLogo (каталог + креатив) */
export const CATALOG_LOGO_AVOID_PROMPT =
  'unbranded products, missing logos, floating logos in air, flat pasted logo overlays, photoshop logo stamps, wireframe logo, dotted logo outline, logo ghost in background, detached logo stickers, duplicate logo panels, white rectangle behind logo, semi-transparent logo overlay on background, logo watermark on scene,';

/** Подсказки после основного промпта — как в каталожном AI */
export const CATALOG_LOGO_INTEGRATION_FOOTER = [
...
function catalogLogoMaterialRules(): string {
  return [
...
/** Промпт для каталожного AI: чистые фото товаров + отдельный реф логотипа */
export function buildCatalogAiLogoReferencePrompt(
  productNames: string[],
  options?: { logoHint?: string; refLayout?: CatalogLogoRefLayout },
): string {
  ...
  if (layout === 'logo-only') {
    ...
    const productLines = productNames
      .map(
        (name, i) =>
          `Product ${i + 1} "${name}" — apply client logo as factory imprint on visible surface only.`,
      )
      .join(' ');
    return [
  ...
  const productLines = productNames.map(
    (name, i) =>
      `Ref ${i + 1} "${name}" — clean catalog photo (shape, color, material only; ignore stock branding on photo).`,
  );
...
/** Блок для image/LLM промптов — обязательное нанесение логотипа по каждому товару */
export function buildLogoApplicationPrompt(
  productNames: string[],
  options?: { hasLogo?: boolean; logoHint?: string },
): string {
  if (!options?.hasLogo) {
...
/** Убирает из LLM image_prompt фразы, противоречащие нанесению логотипа клиента */
export function stripBlankLogoPhrases(prompt: string): string {
  return prompt
