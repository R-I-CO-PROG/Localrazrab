/** Палитра брифа → английский текст для image/LLM промптов */
...
export function hexToColorDescription(hex: string, opts?: { omitHex?: boolean }): string {
  ...
  if (preset) {
    ...
    if (r > 200 && g < 80 && b < 80) result = `red ${hex.toUpperCase()}`;
    else if (b > r + 40 && b > g + 20) result = `blue ${hex.toUpperCase()}`;
    else if (g > r + 30 && g > b + 20) result = `green ${hex.toUpperCase()}`;
    else if (r > 180 && g > 180 && b < 100) result = `yellow ${hex.toUpperCase()}`;
    else if (r > 160 && b > 160 && g < 120) result = `purple ${hex.toUpperCase()}`;
    else if (r > 200 && g > 100 && b < 80) result = `orange ${hex.toUpperCase()}`;
    else result = `color ${hex.toUpperCase()} rgb(${r},${g},${b})`;
  }

  if (!opts?.omitHex) return result;
  return result
...
/** Нейтральные оттенки (ч/б/серый) — не должны быть единственным «брендовым» цветом на всех товарах */
export function isNeutralHex(hex: string): boolean {
  const h = normalizeHex(hex);
  const preset = HEX_PRESETS[h];
  if (preset && /white|black|charcoal|gray|grey/.test(preset)) return true;
...
/** Меньше = выше приоритет для окраски корпуса товара */
function bodyColorPriority(hex: string): number {
  if (!isNeutralHex(hex)) return 0;
  const h = normalizeHex(hex);
  if (h === 'ffffff') return 2;
  if (HEX_PRESETS[h]?.includes('gray')) return 3;
  if (h === '000000' || h === '1a1a1a') return 4;
  return 3;
}

/** Распределяет цвета брифа по товарам — хроматические первыми, чёрный последним */
export function assignBrandColorsToProducts(
...
export function formatPerProductColorAssignments(
  productNames: string[],
  colors: string[] | undefined,
): string {
  ...
  if (!assigned.some(Boolean)) return '';
...
export function formatBrandPalettePrompt(
  colors: string[] | undefined,
...
export function buildPaletteComplianceNegative(colors: string[] | undefined): string {
  const list = (colors ?? []).filter(Boolean);
...
export function enforceBrandColorsInPrompt(
  prompt: string,
  colors: string[] | undefined,
  productNames?: string[],
): string {
  const perProduct =
    productNames && productNames.length > 0 && (colors?.length ?? 0) > 0
  ...
  if (hasHex || hasColorWord) return prompt.trim();

  return `${block} ${prompt.trim()}`.replace(/\s+/g, ' ').trim();
}

export function colorizeProductDescription(
  descriptionEn: string,
  productHex: string | undefined,
): string {
  if (!productHex) return descriptionEn;
