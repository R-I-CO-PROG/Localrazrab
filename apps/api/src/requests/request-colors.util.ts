...
/** Маппинг русских/английских названий цветов → hex */
const COLOR_NAME_TO_HEX: Array<{ keys: string[]; hex: string }> = [
...
const ABSTRACT_PALETTES: Array<{ re: RegExp; hex: string[] }> = [
...
export function colorNameToHex(name: string): string | null {
  ...
  const fromHex = normalizeHex(lower.startsWith('#') ? lower : `#${lower}`);
  if (fromHex) return fromHex;
  for (const { keys, hex } of COLOR_NAME_TO_HEX) {
    if (keys.some((k) => lower.includes(k))) return hex;
  }
  for (const { re, hex } of ABSTRACT_PALETTES) {
    if (re.test(lower)) return hex[0];
  }
  return null;
}

/** Нормализовать colors[] из запроса (имена «зеленый» или hex) в hex-палитру */
export function normalizeRequestColors(colors: unknown): string[] {
  ...
  for (const c of colors) {
    const raw =
      typeof c === 'string'
        ...
        : c && typeof c === 'object' && 'hex' in c
          ? String((c as { hex?: unknown }).hex ?? '').trim()
    ...
    const hex = normalizeHex(raw.startsWith('#') ? raw : `#${raw}`) ?? colorNameToHex(raw);
...
/** Развернуть абстрактные палитры из текста брифа */
export function expandAbstractColorsFromText(text: string): string[] {
