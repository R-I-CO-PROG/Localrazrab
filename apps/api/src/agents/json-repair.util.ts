...
function errorPosition(err: unknown): number | null {
  if (!(err instanceof Error)) return null;
  const m = err.message.match(/position (\d+)/);
  return m ? Number(m[1]) : null;
}

export function repairAgentJsonText(jsonText: string): string {
  let s = jsonText.trim();
...
export function truncateToLastCompleteIdeaObject(jsonText: string, maxPos?: number): string {
  ...
  if (arrayIdx < 0) return closeTruncatedJson(text);
...
function closeTruncatedJson(s: string): string {
  ...
  if (inString) {
    // Обрыв ВНУТРИ незакрытой строки — закрываем её. Раньше здесь безусловно срезалась последняя
    // кавычка (`.replace(/"\s*$/,'')`), из-за чего КОРРЕКТНО завершённый последний элемент рвался:
    // `["a","b","c"` → `["a","b","c` → +`]}` → элемент «c]}». Теперь трогаем строку, только если
    // она действительно не закрыта.
    out += '"';
    // Если это был КЛЮЧ без значения (`{"a":1,"tit`) — снимаем его целиком, иначе `{"a":1,"tit"}`
    // невалиден. Срабатывает только после закрытия оборванной строки, поэтому валидный последний
    // элемент массива (`,"c"`) не затрагивается.
...
export function extractIdeasByRegex(text: string): IdeatorIdea[] {
...
export function extractCriticTopByRegex(text: string): CriticOutput['topIdeas'] {
  const top: CriticOutput['topIdeas'] = [];
  const blockRe =
    /"title"\s*:\s*"((?:[^"\\]|\\.)*)"\s*,\s*"(?:briefFitScore|score)"\s*:\s*(\d+)\s*,\s*"conceptSummary"\s*:\s*"((?:[^"\\]|\\.)*)"/g;
  let m: RegExpExecArray | null;
  while ((m = blockRe.exec(text)) !== null) {
    const title = m[1].replace(/\\"/g, '"').replace(/\\n/g, ' ').trim();
    const conceptSummary = m[3].replace(/\\"/g, '"').replace(/\\n/g, ' ').trim();
    const score = Number(m[2]) || 80;
    if (!title || !conceptSummary) continue;
    top.push({
      title: title.slice(0, 80),
      score,
      briefFitScore: score,
...
export function parseIdeatorOutput(content: string): IdeatorOutput {
  const jsonText = extractJsonObject(content.trim());

  try {
    return parseAgentJson<IdeatorOutput>(content);
  } catch (err) {
    const pos = errorPosition(err);
    ...
    for (const candidate of salvageCandidates) {
      try {
        const parsed = JSON.parse(candidate) as IdeatorOutput;
        if (parsed.ideas?.length >= 12) return parsed;
      } catch {
        /* next */
      }
    }
...
export function parseCatalogIdeatorOutput(content: string): CatalogIdeatorOutput {
  const jsonText = extractJsonObject(content.trim());

  try {
    return parseAgentJson<CatalogIdeatorOutput>(content);
  } catch (err) {
    const pos = errorPosition(err);
    const salvageCandidates = [
      truncateToLastCompleteIdeaObject(jsonText, pos ?? undefined),
    ...
    for (const candidate of salvageCandidates) {
      try {
        const parsed = JSON.parse(candidate) as CatalogIdeatorOutput;
        if (parsed.ideas?.length >= 8) return parsed;
      } catch {
        /* next */
      }
    }
...
export function parseCriticOutput(content: string): CriticOutput {
  const jsonText = extractJsonObject(content.trim());
  let lastErr: unknown;

  try {
    const parsed = parseAgentJson<CriticOutput>(content);
    if (parsed.topIdeas?.length >= 5) return parsed;
  } catch (err) {
    lastErr = err;
  }
  ...
  for (const candidate of salvageCandidates) {
    try {
      const parsed = JSON.parse(candidate) as CriticOutput;
      if (parsed.topIdeas?.length >= 5) return parsed;
    } catch {
      /* next */
    }
