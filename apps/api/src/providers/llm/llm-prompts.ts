...
- image_prompt and composition MUST NOT mention, add or describe ANY product that is not in user_products (no watch, no extra gadget, no suitcase, no food — nothing outside the list). Depict ONLY the exact user_products.
...
export function buildLlmSystemPrompt(respectUserProducts = false): string {
...
/** Свободная генерация: фотореалистичный кадр набора мерча (не улица / не автопарк) */
export function buildLlmSystemPromptForCreative(): string {
...
/** Извлечение параметров брифа из свободного текста клиента */
export function buildLlmSystemPromptForBriefParse(): string {
...
/** Подбор набора: LLM выбирает позиции из каталога с учётом ограничений */
export function buildLlmSystemPromptForSuggest(): string {
...
/** 5 разных наборов из каталога для выбора клиентом */
export function buildLlmSystemPromptForCatalogConcepts(): string {
  return `You are a senior corporate merchandise curator specializing in branded gift sets.
...
/** Подбор дополнительных товаров к существующему набору */
export function buildLlmSystemPromptForProductAdd(): string {
...
export function resolveLlmSystemPrompt(
  input: {
    sceneOnly?: boolean;
  ...
  if (input.briefParseMode) return buildLlmSystemPromptForBriefParse();
  if (input.productAddMode) return buildLlmSystemPromptForProductAdd();
  if (input.catalogConceptsMode) return buildLlmSystemPromptForCatalogConcepts();
  if (input.suggestMode) return buildLlmSystemPromptForSuggest();
  if (input.creativeMode) return buildLlmSystemPromptForCreative();
  if (input.sceneOnly) return buildLlmSystemPromptForScene();
  return buildLlmSystemPrompt(respectUser);
}

/** @deprecated use buildLlmSystemPrompt() */
export const LLM_SYSTEM_PROMPT = buildLlmSystemPrompt(false);

export function buildLlmUserMessage(input: Record<string, unknown>): string {
  return JSON.stringify(input, null, 2);
}

export function buildLlmUserPayload(
  input: {
    userPrompt: string;
    category: string;
  ...
  const desiredItemCount = input.desiredItemCount ?? defaultItemCount(input.userPrompt);
  const budgetPerSet =
    ...
    ...(!input.catalogConceptsMode && !options?.productAddMode
      ? {
          set_composition: {
            ...
            rule:
              'Набор = разные роли. Каждый тип товара максимум один раз, если бриф явно не просит несколько одинаковых позиций. Товары должны дополнять друг друга и соответствовать task.',
          },
        }
      : {}),
    ...(input.catalogConceptsMode
      ? {
          category_diversity: {
            ...
            optional_type_max_concepts: OPTIONAL_TYPE_MAX_CONCEPTS,
            mandatory_type_max_concepts: MANDATORY_TYPE_MAX_CONCEPTS,
            mandatory_types_from_brief: (input.mandatoryConceptTypes ?? []).map((slug) => ({
              type: slug,
              label: conceptTypeLabel(slug),
            })),
            rule:
              'Never repeat the same catalog SKU across the 5 concepts. Within each set: max 1 item per product type. Prefer thematic fit to concept title/composition.',
          },
        }
      : {}),
  };
