...
export interface ShortlistContext {
  ...
  /** #6 ЦВЕТО-СТАРВАЦИЯ: если строгий цвет-гейт (colorCriticalClash) голодит набор ниже min
   *  (фирм. цвет клешит с темой каталога — уютные товары сине-серые, а бренд red/green/yellow),
   *  пропускаем цвето-клеш: лучше полный тематичный набор в неидеальном цвете, чем 1 позиция. */
  relaxColorClash?: boolean;
  /** Материал ВСЕГО набора из LLM-классификации намерения брифа («полностью кожаный»). Реальный
   *  бонус в scoreRow (catalog-shortlist.util.ts) — сильное предпочтение, НЕ жёсткий фильтр:
   *  аудитория/архетип/связность важнее (см. её же scoreArchetypeMatch/scoreConceptCoherence),
   *  материал лишь склоняет выбор среди уже тематически подходящих кандидатов. */
