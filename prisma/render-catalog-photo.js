...
exports.renderCatalogPhoto = renderCatalogPhoto;
const fs_1 = require("fs");
const path_1 = require("path");
const sharp_1 = __importDefault(require("sharp"));
async function renderCatalogPhoto(opts) {
    const card = 512;
    const productSize = 340;
    if (!(0, fs_1.existsSync)(opts.silhouettePath)) {
        throw new Error(`Silhouette not found: ${opts.silhouettePath}`);
    }
    (0, fs_1.mkdirSync)((0, path_1.dirname)(opts.outputPath), { recursive: true });
    const bgSvg = `
  ...
  `;
    const silhouette = await (0, sharp_1.default)(opts.silhouettePath)
    ...
    const productShape = await (0, sharp_1.default)({
  ...
  `;
    const productLit = await (0, sharp_1.default)(productShape)
  ...
  `;
    const shadow = await (0, sharp_1.default)(Buffer.from(shadowSvg)).blur(5).png().toBuffer();
  ...
  `;
    await (0, sharp_1.default)(Buffer.from(bgSvg))
        .composite([
...
//# sourceMappingURL=render-catalog-photo.js.map